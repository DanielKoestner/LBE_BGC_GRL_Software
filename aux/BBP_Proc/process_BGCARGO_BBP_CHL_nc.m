%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
function [Data_bbp_proc,Data_chl_proc] = process_BGCARGO_BBP_CHL(Data_CHL,Data_BBP,path_oneargo,path_to_nc_files,varargin);
...
% apply correction to obtain BBP_s;
  ...
  fname = char(fieldnm); 
  fname = fname(2:end);
  PRES = ncread([path_to_nc_files,'/',fname,'_Sprof.nc'],var_pres);
  LATITUDE = ncread([path_to_nc_files,'/',fname,'_Sprof.nc'],'LATITUDE');
  LONGITUDE = ncread([path_to_nc_files,'/',fname,'_Sprof.nc'],'LONGITUDE');
 

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'BBP700_s';
  try
  ...
  netcdf.close(ncid) 
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_s ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_s", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_s',Float_Struc_BBP.BBP700_s);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'PROFILE_BBP700_s_QC';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v PROFILE_BBP700_s_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"PROFILE_BBP700_s_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'PROFILE_BBP700_s_QC',Float_Struc_BBP.PROFILE_BBP700_QC);
  ...
  netcdf.close(ncid) 
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_s_dPRES ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_s_dPRES", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_s_dPRES',Float_Struc_BBP.BBP700_dPRES);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'BBP700_s_ADJUSTED';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_s_ADJUSTED ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_s_ADJUSTED", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_s_ADJUSTED',Float_Struc_BBP.BBP700_ADJUSTED);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'BBP700_s_ADJUSTED_QC';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_s_ADJUSTED_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_s_ADJUSTED_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");
  BBP700_ADJUSTED_s_QC = num2str(Float_Struc_BBP.BBP700_ADJUSTED_QC(:,1));
  for j=2:length(LATITUDE);
    BBP700_ADJUSTED_s_QC_temp = Float_Struc_BBP.BBP700_ADJUSTED_QC(:,j);
    BBP700_ADJUSTED_s_QC = horzcat(BBP700_ADJUSTED_s_QC,num2str(BBP700_ADJUSTED_s_QC_temp));
  end
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_s_ADJUSTED_QC',BBP700_ADJUSTED_s_QC);


  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'BBP700_s_ADJUSTED_ERROR';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_s_ADJUSTED_ERROR ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_s_ADJUSTED_ERROR", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_s_ADJUSTED_ERROR',Float_Struc_BBP.BBP700_ADJUSTED_ERROR);
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_s_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_s_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");

  BBP700_s_QC = num2str(Float_Struc_BBP.BBP700_QC(:,1));
  for j=2:length(LATITUDE);
    BBP700_s_QC_temp = Float_Struc_BBP.BBP700_QC(:,j);
    BBP700_s_QC = horzcat(BBP700_s_QC,num2str(BBP700_s_QC_temp));
  end 
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_s_QC',BBP700_s_QC);
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_l ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_l", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_l',Float_Struc_BBP.BBP700_l);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'PROFILE_BBP700_l_QC';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v PROFILE_BBP700_l_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"PROFILE_BBP700_l_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'PROFILE_BBP700_l_QC',Float_Struc_BBP.PROFILE_BBP700_QC);
  ...
  netcdf.close(ncid) 
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_l_dPRES ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_l_dPRES", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_l_dPRES',Float_Struc_BBP.BBP700_dPRES);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'BBP700_l_ADJUSTED';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_l_ADJUSTED ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_l_ADJUSTED", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_l_ADJUSTED',Float_Struc_BBP.BBP700_ADJUSTED);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'BBP700_l_ADJUSTED_QC';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_l_ADJUSTED_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_l_ADJUSTED_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");
  BBP700_ADJUSTED_l_QC = num2str(Float_Struc_BBP.BBP700_ADJUSTED_QC(:,1));
  for j=2:length(LATITUDE);
    BBP700_ADJUSTED_l_QC_temp = Float_Struc_BBP.BBP700_ADJUSTED_QC(:,j);
    BBP700_ADJUSTED_l_QC = horzcat(BBP700_ADJUSTED_l_QC,num2str(BBP700_ADJUSTED_l_QC_temp));
  end
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_l_ADJUSTED_QC',BBP700_ADJUSTED_l_QC);


  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'BBP700_l_ADJUSTED_ERROR';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_l_ADJUSTED_ERROR ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_l_ADJUSTED_ERROR", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_l_ADJUSTED_ERROR',Float_Struc_BBP.BBP700_ADJUSTED_ERROR);
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v BBP700_l_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"BBP700_l_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");

  BBP700_l_QC = num2str(Float_Struc_BBP.BBP700_QC(:,1));
  for j=2:length(LATITUDE);
    BBP700_l_QC_temp = Float_Struc_BBP.BBP700_QC(:,j);
    BBP700_l_QC = horzcat(BBP700_l_QC,num2str(BBP700_l_QC_temp));
  end 
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'BBP700_l_QC',BBP700_l_QC);
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_l_ADJUSTED ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_l_ADJUSTED", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_l_ADJUSTED',Float_Struc_Chl.chl_l);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'PROFILE_CHLA_l_QC';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v PROFILE_CHLA_l_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"PROFILE_CHLA_l_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'PROFILE_CHLA_l_QC',Float_Struc_Chl.PROFILE_CHLA_QC);
  ...
  netcdf.close(ncid) 
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_l_dPRES ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_l_dPRES", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_l_dPRES',Float_Struc_Chl.CHLA_dPRES);
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_l ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_l", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_l',Float_Struc_Chl.chl_l);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'CHLA_l_ADJUSTED_QC';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_l_ADJUSTED_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_l_ADJUSTED_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");
  CHLA_l_ADJUSTED_QC = num2str(Float_Struc_Chl.CHLA_ADJUSTED_QC(:,1));
  for j=2:length(LATITUDE);
    CHLA_l_ADJUSTED_QC_temp = Float_Struc_Chl.CHLA_ADJUSTED_QC(:,j);
    CHLA_l_ADJUSTED_QC = horzcat(CHLA_l_ADJUSTED_QC,num2str(CHLA_l_ADJUSTED_QC_temp));
  end
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_l_ADJUSTED_QC',CHLA_l_ADJUSTED_QC);


  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'CHLA_l_ADJUSTED_ERROR';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_l_ADJUSTED_ERROR ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_l_ADJUSTED_ERROR", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_l_ADJUSTED_ERROR',Float_Struc_Chl.CHLA_ADJUSTED_ERROR);
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_l_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_l_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");

  CHLA_l_QC = num2str(Float_Struc_Chl.CHLA_QC(:,1));
  for j=2:length(LATITUDE);
    CHLA_l_QC_temp = Float_Struc_Chl.CHLA_QC(:,j);
    CHLA_l_QC = horzcat(CHLA_l_QC,num2str(CHLA_l_QC_temp));
  end 
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_l_QC',CHLA_l_QC);
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_s_ADJUSTED ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_s_ADJUSTED", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_s_ADJUSTED',Float_Struc_Chl.chl_s);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'PROFILE_CHLA_s_QC';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v PROFILE_CHLA_s_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"PROFILE_CHLA_s_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'PROFILE_CHLA_s_QC',Float_Struc_Chl.PROFILE_CHLA_QC);
  ...
  netcdf.close(ncid) 
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_s_dPRES ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_s_dPRES", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_s_dPRES',Float_Struc_Chl.CHLA_dPRES);
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_s ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_s", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_s',Float_Struc_Chl.chl_s);

  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'CHLA_s_ADJUSTED_QC';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_s_ADJUSTED_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_s_ADJUSTED_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");
  CHLA_s_ADJUSTED_QC = num2str(Float_Struc_Chl.CHLA_ADJUSTED_QC(:,1));
  for j=2:length(LATITUDE);
    CHLA_s_ADJUSTED_QC_temp = Float_Struc_Chl.CHLA_ADJUSTED_QC(:,j);
    CHLA_s_ADJUSTED_QC = horzcat(CHLA_s_ADJUSTED_QC,num2str(CHLA_s_ADJUSTED_QC_temp));
  end
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_s_ADJUSTED_QC',CHLA_s_ADJUSTED_QC);


  ncid = netcdf.open([path_to_nc_files,'/',fname,'_Sprof.nc'],'nowrite');
  strv = 'good';
  var = 'CHLA_s_ADJUSTED_ERROR';
  try
    ID = netcdf.inqVarID(ncid,var);
  catch exception
    if strcmp(exception.identifier,'MATLAB:imagesci:netcdf:libraryFailure')
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_s_ADJUSTED_ERROR ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_s_ADJUSTED_ERROR", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Format","classic");
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_s_ADJUSTED_ERROR',Float_Struc_Chl.CHLA_ADJUSTED_ERROR);
  ...
  netcdf.close(ncid)
  if strcmp(strv,'good') == 1;
    system(['ncks -O -x -v CHLA_s_QC ',path_to_nc_files,'/',fname,'_Sprof.nc ',path_to_nc_files,'/',fname,'_Sprof.nc']);
  end

  nccreate([path_to_nc_files,'/',fname,'_Sprof.nc'],"CHLA_s_QC", ...
         "Dimensions",{"N_LEVELS",length(PRES),"N_PROF",length(LATITUDE)},"Datatype","char","Format","classic");

  CHLA_s_QC = num2str(Float_Struc_Chl.CHLA_QC(:,1));
  for j=2:length(LATITUDE);
    CHLA_s_QC_temp = Float_Struc_Chl.CHLA_QC(:,j);
    CHLA_s_QC = horzcat(CHLA_s_QC,num2str(CHLA_s_QC_temp));
  end 
  ncwrite([path_to_nc_files,'/',fname,'_Sprof.nc'],'CHLA_s_QC',CHLA_s_QC);
