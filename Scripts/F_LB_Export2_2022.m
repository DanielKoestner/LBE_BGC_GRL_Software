...
% b1=bar(Ez_in,'FaceColor',colors(5,:));
% b2=bar(Ez2_in,'FaceColor',colors(2,:),'FaceAlpha',0.7,'linestyle',':');

b2=bar(Ez2_in,'FaceColor',coltz2,'FaceAlpha',0.8,'linestyle',':','linewidth',1.25);
b1=bar(Ez_in,'FaceColor',coltz,'FaceAlpha',0.6,'edgecolor',[0.5 0.5 0.7],'linewidth',1.5);


p1=plot(1:12,P_in,'--','Color',colp,'linewidth',2.75);
% p1=plot(1:12,P_in,'--','Color',[0.7646 0.7629 0.5230],'linewidth',2.5);
...
[hl,lines]=legendflex([p1 b1 b2 p1],{'PZ','TZ','lower-TZ'}, 'ref', gca, ...
'anchor', {'nw', 'nw'}, 'buffer', [5 -2], 'xscale', 0.5, 'ncol',1,'nrow',3, ...
'box', 'off', 'FontSize', fs-3);
PatchInLegend = findobj(hl, 'type', 'patch');
PatchInLegend(2).FaceAlpha=0.6;
PatchInLegend(1).FaceAlpha=0.85;

% text(0.7,0.87,sprintf('\\Sigma \\itE^{PZ}\\rm = %1.0f (%1.0f – %1.0f) mg m^{-2} y^{-1}',sum(P_in)*365,sum(P_in_err(1,:))*365,sum(P_in_err(2,:))*365),'units','normalized','color',[0.4 0.75 0.4],'fontsize',fs-4)
% text(0.7,0.72,sprintf('\\Sigma \\itE^{TZ}\\rm = %1.0f (%1.0f – %1.0f) mg m^{-2} y^{-1}',sum(Ez_in)*365,sum(Ez_in_err(1,:))*365,sum(Ez_in_err(2,:))*365),'units','normalized','color',colors(4,:),'fontsize',fs-4)
% text(0.76,0.695,sprintf('%1.2f (%1.1f – %1.1f) g m^{-2} yr^{-1}',sum(Ez2_in)*365/1000,sum(Ez2_in_err(1,:))*365/1000,sum(Ez2_in_err(2,:))*365/1000),'units','normalized','color',colors(2,:),'fontsize',fs-4)
...
hold on
ha1(2).YGrid='on';
% b1=bar(Ez_out,'FaceColor',colors(5,:));
% b2=bar(Ez2_out,'FaceColor',colors(2,:),'FaceAlpha',0.7,'linestyle',':');
b2=bar(Ez2_out,'FaceColor',coltz2,'FaceAlpha',0.8,'linestyle',':','linewidth',1.25);
b1=bar(Ez_out,'FaceColor',coltz,'FaceAlpha',0.6,'edgecolor',[0.5 0.5 0.7],'linewidth',1.5);
p1=plot(1:12,P_out,'--','Color',colp,'linewidth',2.75);
% p1=plot(1:12,P_out,'--','Color',[0.7646 0.7629 0.5230],'linewidth',2.5);
...
x=1:6;
y=TE_out(1:6); % set to shallowest depth horizon (100 or 200)
ydeep=TE_out_deep(1:6); %set to deepest depth horizon (400 or 500)
% ydeep(12)=y(12);
xx = [x;x];
yy = [y;ydeep];
...
p2=plot(1:6,TE_out(1:6),'color',colors2(3,:),'linewidth',0.75,'marker','s','MarkerFaceColor',colors2(3,:),'MarkerSize',1,'MarkerEdgeColor','none');
...
ydeep=TE_in_deep; %set to deepest depth horizon (400 or 500)
xx = [x;x];
yy = [y;ydeep];
alps=yy*0.5; alps(alps<0)=0; alps(alps>0.4)=0.4;
% alps=[ones(1,12)*0.4;ones(1,12)*0.3];
...
plot(1:12,TE_in_deep,'color',coldeep,'linewidth',0.1);
