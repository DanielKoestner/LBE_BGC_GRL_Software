...
while cnt<=length(lb)
    ...
    for i = 1:length(lb{cnt}.dnum)
        dnums=[dnums; lb{cnt}.dnum(i)];
        [eddy_id]=in_eddy(lb{cnt}.dnum(i),lb{cnt}.lat(i),lb{cnt}.lon(i),'LBE_locations.mat');
        if eddy_id==1 %& month(datetime(lb{cnt}.dnum(i),'ConvertFrom','datenum'))==8
            m_plot(lb{cnt}.lon(i),lb{cnt}.lat(i),'lines','none','marker','o','markerfacecolor',floatcol,'markeredgecolor','none','markersize',3,'linewidth',0.25);
            ...
            m_plot(lb{cnt}.lon(i),lb{cnt}.lat(i),'lines','none','marker','o','markerfacecolor',floatcol,'markeredgecolor','none','markersize',1.75,'linewidth',0.25);
...
while cnt<=length(lb)
    ...
    floatcol=colors(cnt,:);
    m_plot(lb{cnt}.lon(end),lb{cnt}.lat(end),'marker','o','markerfacecolor',floatcol,'markersize',7,'markeredgecolor',[0 0 0]);
...
if print_flag==1
    figure(hf1);
    print('Figures/V3/LB_FloatMap_2022.pdf','-dpdf','-r800')
    figure(hf2);
    print('Figures/V3/LB_MonthlyYrsStats_2022.pdf','-dpdf','-r800')
%     exportgraphics(hf1,['Figures/OSM24_LB_Map.pdf'],'ContentType','vector');

    figure(hf3);
    print('Figures/V3/LB_GlobalMap_2022.pdf','-dpdf','-r800')
end
